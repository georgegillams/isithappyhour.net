...
function getMeta(cb) {
  ...
  download.on('end', () => {
    const metaData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../server_content/greasemonkey', 'scripts.json'), 'utf8')
    );
    cb(metaData);
  });
}
...
function sendGreasemonkeyFile(scriptId, req, res) {
  try {
    ...
    res.status(500).send({ error: 'An error occurred fetching resources from GitHub.' });
...
router.get(`/api/browser-scripts/*`, (req, res) => {
  const pathValues = req.path.split('/');
  const scriptId = pathValues[pathValues.length - 1];
  sendGreasemonkeyFile(scriptId, req, res);
});
